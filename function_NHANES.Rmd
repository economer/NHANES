...
one can download more than one dataset if it is required. You have to use "|" to distinguish between the datasets inside data_name argument. For instance, if you require datasets including information about body measures and blood pressure you could use the function as follows: 

download_nh(data_name = c("blood pressure|body measure"),year = "1999-2000")
...
download_nh(data_name = ,year = "1999-2000")
...
download_nh(data_name = c("blood pressure|body measure"),year = "1999-2000")
file_list3 %>%
  filter(str_detect(string = file_name,pattern = c(".*blood pressure|.*body measure.*")),
         Year=="1999-2000")
```
